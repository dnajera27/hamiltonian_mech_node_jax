# -*- coding: utf-8 -*-
"""noAnalyt_noAE_2DOF_UnforcedUndamped.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ZUM1HYSrxpoxA5qfIsEjzF_aX8apHM3N
"""
...
def oscillator(w,t,p):
    x1,x2,xdot1,xdot2 = w
    m1,m2,k1,k2,k_prime,beta1,beta2,F0,omg = p
    
    xdotdot1 = (F0*np.cos(omg*t) + k_prime*(x2 - x1)**3 - k1*x1 - beta1*xdot1)/m1
    xdotdot2 = (-k2*x2 + k_prime*(x1 - x2)**3 - beta2*xdot2)/m2
...
p = [m1,m2,k1,k2,k_prime,beta1,beta2,F0,omg]
...
dir_name = "/content/drive/MyDrive/UCSD_research/jax/oscillator/chkpts_2DOF_UnforcedUndamped_noAE_noAnalyt/"
...
#R = predict_Rayleigh(state.params, normTrainData[:Nsim,:,:])
...
#plt.subplot(2,2,1)
plt.plot(normTrainData[:Nint,:,0], normTrainData[:Nint,:,1], linewidth = 5, alpha = 0.6)
...
normTrainData[:,:,0].shape

mat_params = [m1,m2,k1,k2,k_prime,beta1,beta2,F0,omg]

def kinetic_energy(params,x,dx):
  [m1,m2,k1,k2,k_prime,beta1,beta2,F0,omg] = params
  dx1 = dx[0]
  dx2 = dx[1]
  T = 1/2*m1*dx1**2 + 1/2*m2*dx2**2

  return T

def potential_energy(params,x,dx):
  [m1,m2,k1,k2,k_prime,beta1,beta2,F0,omg] = params
  x1 = x[0]
  x2 = x[1]
  U = 1/2*k1*x1**2 + 1/2*k2*x2**2 + 1/4*k_prime*(x2 - x1)**4

  return U  

def rayleigh_energy(params,x,dx):
  [m1,m2,k1,k2,k_prime,beta1,beta2,F0,omg] = params
  dx1 = dx[0]
  dx2 = dx[1]
  R = 1/2*beta1*dx1**2 + 1/2*beta2*dx2**2 

  return R  

def get_Mass_Matrix_analyt(params, q, dq):
  latent = LATENT
  M = jax.hessian(kinetic_energy, argnums = 2)(params, q, dq)
  M = jnp.reshape(M, (latent, latent))
  return M 

def get_K_Matrix_analyt(params, q, dq):
  latent = LATENT
  M = jax.hessian(potential_energy, argnums = 1)(params, q, dq)
  M = jnp.reshape(M, (latent, latent))
  return M 

def get_C_Matrix_analyt(params, q, dq):
  latent = LATENT
  M = jax.hessian(rayleigh_energy, argnums = 2)(params, q, dq)
  M = jnp.reshape(M, (latent, latent))
  return M 

def get_A(params, q, dq):
  latent = LATENT
  M = get_Mass_Matrix_analyt(params, q, dq)
  K = get_K_Matrix_analyt(params, q, dq)
  C = get_C_Matrix_analyt(params, q, dq)

  A1 = jnp.concatenate([jnp.zeros((latent, latent)), jnp.eye(latent)], axis = 1)
  A2 = jnp.concatenate([-jnp.linalg.pinv(M) @ K, -jnp.linalg.pinv(M) @ C], axis = 1)

  A = jnp.concatenate([A1, A2], axis = 0)

  return A

def get_MinvK(params, q, dq):
  latent = LATENT
  M = get_Mass_Matrix_analyt(params, q, dq)
  K = get_K_Matrix_analyt(params, q, dq)
  C = get_C_Matrix_analyt(params, q, dq)

  MinvK = jnp.linalg.pinv(M) @ K

  return MinvK

def get_MinvC(params, q, dq):
  latent = LATENT
  M = get_Mass_Matrix_analyt(params, q, dq)
  K = get_K_Matrix_analyt(params, q, dq)
  C = get_C_Matrix_analyt(params, q, dq)

  MinvC = jnp.linalg.pinv(M) @ C

  return MinvC

kin = jax.vmap(kinetic_energy, in_axes = (None,0,0))
pot = jax.vmap(potential_energy, in_axes = (None,0,0))
ray = jax.vmap(rayleigh_energy, in_axes = (None,0,0))

Man = jax.vmap(get_Mass_Matrix_analyt, in_axes = (None,0,0))
Kan = jax.vmap(get_K_Matrix_analyt, in_axes = (None,0,0))
Can = jax.vmap(get_C_Matrix_analyt, in_axes = (None,0,0))

M_analyt = Man(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])
K_analyt = Kan(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])
C_analyt = Can(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])

T_analyt = kin(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])
U_analyt = pot(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])
R_analyt = ray(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])

MinvK_ = jax.vmap(get_MinvK, in_axes = (None,0,0))
MinvK = MinvK_(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])

MinvC_ = jax.vmap(get_MinvC, in_axes = (None,0,0))
MinvC = MinvC_(mat_params, normTrainData[:,:,0], normTrainData[:,:,1])

Vk,Dk = jax.vmap(jnp.linalg.eigh, in_axes = 0)(MinvK)
Vc,Dc = jax.vmap(jnp.linalg.eigh, in_axes = 0)(MinvC)

#Man.shape

V_m_analyt, D_m_analyt = jax.vmap(jnp.linalg.eigh, in_axes = 0)(M_analyt)
V_k_analyt, D_k_analyt = jax.vmap(jnp.linalg.eigh, in_axes = 0)(K_analyt)

V_m_nn, D_m_nn = jax.vmap(jnp.linalg.eigh, in_axes = 0)(M)
V_k_nn, D_k_nn = jax.vmap(jnp.linalg.eigh, in_axes = 0)(K)

print(D_k_analyt.shape)
...
for ii in range(M.shape[0]):
  Vk_nn_, Dk_nn_ = np.linalg.eigh(np.linalg.pinv(M[ii]) @ K[ii])
  Vk_nn.append(Vk_nn_)
  Dk_nn.append(Dk_nn_)
  Vc_nn_, Dc_nn_ = np.linalg.eigh(np.linalg.pinv(M[ii]) @ C[ii])
  Vc_nn.append(Vc_nn_)
  Dc_nn.append(Dc_nn_)

Vk_nn = np.array(Vk_nn)
Dk_nn = np.array(Dk_nn)
Vc_nn = np.array(Vc_nn)
Dc_nn = np.array(Dc_nn)

print(Vk_nn.shape, Dk_nn.shape)
...
cols = sns.cubehelix_palette(100, start=.5, rot=-.5)
for ii in range(100):
    
    plt.subplot(1,2,1)
    plt.title('$eig[{M}(x, \dot{x})]$')
    eig_vector_1 = np.vstack([origin, D_m_nn[ii][:,0]]).T
    eig_vector_2 = np.vstack([origin, D_m_nn[ii][:,1]]).T

    plt.plot(eig_vector_1[0],
            eig_vector_1[1], c = cols[ii], alpha = 0.8)

    plt.plot(eig_vector_2[0],
            eig_vector_2[1], c = cols[ii], alpha = 0.8)


    eig_vector_1 = np.vstack([origin, D_m_analyt[ii][:,0]]).T
    eig_vector_2 = np.vstack([origin, D_m_analyt[ii][:,1]]).T

    plt.plot(eig_vector_1[0],
            eig_vector_1[1], 'r--', alpha = 0.6, linewidth = 3)

    plt.plot(eig_vector_2[0],
            eig_vector_2[1], 'r--', alpha = 0.6, linewidth = 3)
    
    plt.subplot(1,2,2)
    plt.title('$eig[{K}(x, \dot{x})]$')
    eig_vector_1 = np.vstack([origin, D_k_nn[ii][:,0]]).T
    eig_vector_2 = np.vstack([origin, D_k_nn[ii][:,1]]).T
    ...
    eig_vector_1 = np.vstack([origin, D_k_analyt[ii][:,0]]).T
    eig_vector_2 = np.vstack([origin, D_k_analyt[ii][:,1]]).T
...
plt.tight_layout()
plt.savefig('/content/drive/MyDrive/UCSD_research/jax/oscillator/figures/2DOF_UnforcedUndamped_time.png')
...
plt.scatter(t[:Nint], (testData[:Nint,0,0] - xpred[:,0]))
plt.ylabel('Displacement')
plt.xlabel('Time, s')

plt.subplot(2, 1, 2)

plt.plot(t[:Nint], (testData[:Nint,1,0] - xpred[:,1]), 'k.')
plt.ylabel('Velocity')
plt.xlabel('Time, s')
#plt.xlim([0, 0.1])

plt.tight_layout()
plt.savefig('2DOF_UnforcedUndamped_time_resid.png')
...
plt.tight_layout()
plt.savefig('/content/drive/MyDrive/UCSD_research/jax/oscillator/figures/2DOF_UnforcedUndamped_phase.png')
plt.show()

plt.figure(figsize = (12,8))
ax = plt.subplot(111)
xti = np.array([1,2,3,4])
ax.bar(xti-0.2, freqsHz, width=0.4, color='b', align='center', label = 'Actual')
ax.bar(xti+0.2, freqsHz_nn, width=0.4, color='g', align='center', label = 'Neural Network')
plt.ylabel('Freq, Hz')
plt.legend()
...
x_ticks_labels = ['Mode 1', 'Mode 2', 'Mode 3', 'Mode 4']
...
cross_ortho = np.dot(dnorm.T, np.dot(Mmat, dnn_norm))**2
